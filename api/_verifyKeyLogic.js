...
// KEYS[1] = access key, ARGV[1] = current ISO timestamp for expiry check, ARGV[2] = exhausted TTL in seconds.
...
const EXHAUSTED_TTL_SEC = 90 * 86400;
...
export async function verifyAndDecrementKey(key) {
  const raw = await withTimeout(
    redis.eval(LUA_SCRIPT, [key], [new Date().toISOString(), String(EXHAUSTED_TTL_SEC)]),
    5000, 'verify key'
  );
